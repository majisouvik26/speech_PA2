### option-1: multiprocessing with cpu
...
## simple python code multiprocessing

from feature_extraction import extract_mfcc, compute_aggregated_features
import pandas as pd
from tqdm import tqdm

def process_file(file, label, n_mfcc):
    try:
        mfcc, sr = extract_mfcc(file, n_mfcc=n_mfcc)
        if mfcc is None:
            return None
...
def create_feature_dataframe(audio_files, labels, n_mfcc):
    ...
    features = []
    for file, label in tqdm(zip(audio_files, labels), total=len(audio_files), desc="Extracting Features"):
        res = process_file(file, label, n_mfcc)
        if res is not None:
            features.append(res)
